...
public class ArmControllerScript : MonoBehaviour {
	...
	{  
		...
		public string bloodImpactTag = "Blood";
	}
	...
	void Update () {
		...
		if (_starterAssetsInputs.shooting && !ShootSettings.automaticFire
			...
			} else if (MeleeSettings.isMeleeWeapon)
			{
				MeleeShoot();
			}
	...
	void ShotgunShoot() {
		...
		if (!ReloadSettings.casingOnReload && Prefabs.casingPrefab != null) {
		...
		for (int i = 0; i < ShootSettings.pellets; ++i) {
			...
			if (Physics.Raycast (Spawnpoints.bulletSpawnPoint.transform.position, direction, 
			                     out hit, ShootSettings.bulletDistance,_layerMask)) {
	...
	{
		if ( isReloading || isDrawing || isRunning || isJumping)
		{
			return false;
		}
