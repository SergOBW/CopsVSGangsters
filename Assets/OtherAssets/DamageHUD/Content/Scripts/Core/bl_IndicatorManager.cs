...
public class bl_IndicatorManager : MonoBehaviour {
    ...
    {
        GameObject newentry = Instantiate(IndicatorUI);
    ...
    void ControllIndicators()
    {
        for(int i = 0; i < IndicatorsEntrys.Count; i++)
        {
            ...
            if(indicator == null || indicator.Transform == null || indicator.Sender == null)
            {
    ...
    public void SetupIndicators()
    {
        if (IndicatorsEntrys.Count > 0)
        {
            foreach (var blIndicator in IndicatorsEntrys)
            {
                Destroy(blIndicator.gameObject);
            }
        }

        List<Transform> childs = new List<Transform>();
        foreach (Transform child in PanelIndicator.transform)
        {
            childs.Add(child);
        }
        ...
        IndicatorsEntrys = new List<bl_Indicator>();
    }
